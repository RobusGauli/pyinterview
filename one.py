class VLanNode:
    ...
    def exists_primary_secondary(self, device_id):
        ...
        return set(self.device_primary.keys()).intersection(set(self.device_secondary.keys())).intersection(set([device_id]))
    
    def common_device_with_lower_value(self, vlan_id):
        for device in sorted(self.device_common.values(), key=lambda dev: int(dev.device_id)):
            if vlan_id not in device.reserved_vlan_ids:
                return device
    
    def primary_device_with_lower_value(self, vlan_id):
        for device in sorted(self.device_secondary.values(), key=lambda dev: int(dev.device_id)):
            if vlan_id not in device.reserved_vlan_ids:
                return device
...
def main():
    ...
    perform_computation(g, requests, vlan_ids)
